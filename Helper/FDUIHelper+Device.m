...
#import "FDLayoutCommonDefines.h"
#import "NSObject+PerformSelector.h"
#import "NSString+FDHandle.h"

@interface _FDUIPortraitViewController : UIViewController
@end

@implementation _FDUIPortraitViewController
...
@implementation FDUIHelper (Device)
...
+ (BOOL)isNotchedScreen {
    if (isNotchedScreen < 0) {
        if (@available(iOS 12.0, *)) {
            ...
            [[UIScreen mainScreen] fd_performSelector:peripheryInsetsSelector withPrimitiveReturnValue:&peripheryInsets];
            if (peripheryInsets.bottom <= 0) {
                ...
                if (peripheryInsets.bottom <= 0) {
                    ...
                    _FDUIPortraitViewController *viewController = [_FDUIPortraitViewController new];
...
+ (BOOL)is65InchScreen {
    if (is65InchScreen < 0) {
        ...
        is65InchScreen = (DEVICE_WIDTH == self.screenSizeFor65Inch.width && DEVICE_HEIGHT == self.screenSizeFor65Inch.height && !FDUIHelper.is61InchScreen) ? 1 : 0;
    }
...
+ (BOOL)isHighPerformanceDevice {
    if (isHighPerformanceDevice < 0) {
        NSString *model = [self deviceModel];
        NSString *identifier = [model fd_stringMatchedByPattern:@"\\d+"];
